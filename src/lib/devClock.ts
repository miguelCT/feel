 ...
 * touching `lineup.json`.
 *
 * Usage via URL query param:
...
/**
 * Milliseconds to add to `Date.now()`. 0 unless a time-travel override is
 * active.
 */
...
/** Whether a time override is currently active (for the preview banner). */
export const isTimeTravelActive = (): boolean => getClockOffset() !== 0;
